...
public class Main {
     ...
     *
     * <p>Esperado: cada argumento no formato {@code col,row;expected,fixed}
     * — por exemplo: {@code 0,0;5,true} significa coluna 0, linha 0,
     * valor esperado 5, célula fixa.</p>
     */
    ...
    private static void startGame(final Map<String, String> positions) {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                // Cada argumento: "col,row;expected,fixed"
                // positions.get("col,row") → "expected,fixed"
    ...
    private static void removeNumber() {
        ...
        if (!board.clearValue(col, row)) {
            System.out.printf("A posição [%s,%s] tem um valor fixo e não pode ser removida.%n", col, row);
        }
    }
    ...
    private static void showCurrentGame() {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            for (var col : board.getSpaces()) {
                var actual = col.get(i).getActual();
    ...
    private static void clearGame() {
        ...
        if (confirm.equalsIgnoreCase("sim")) {
            board.reset();
            System.out.println("O jogo foi limpo. Apenas os valores fixos foram mantidos.");
        }
    }
